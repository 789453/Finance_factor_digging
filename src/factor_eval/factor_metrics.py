...
class FactorMetricsEvaluator:
    ...
    def evaluate_tensor(self, factor: torch.Tensor, target: torch.Tensor, returns: Optional[torch.Tensor] = None, direction: float = 1.0) -> Dict[str, Any]:
        """
        根据因子值和目标值计算各项指标。
        factor, target: [n_dates, n_assets]
        """
        from .panel_ops import (
            cross_sectional_ic, cross_sectional_rank_ic, coverage_by_day,
            compute_stability_score, compute_topk_returns
        )
        ...
        # 4. 新增稳定性指标 (基于 IC)
        stability = compute_stability_score(torch.tensor(daily_ic, device=self.device))
        
        # 5. TopK 收益指标 (如果提供了收益序列)
        topk_ret = 0.0
        topk_ir = 0.0
        if returns is not None:
            returns = returns.to(self.device)
            daily_topk_ret = compute_topk_returns(factor, returns)
            topk_ret = np.nanmean(daily_topk_ret)
            topk_std = np.nanstd(daily_topk_ret)
            topk_ir = topk_ret / (topk_std + 1e-8)
