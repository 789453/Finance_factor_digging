...
try:
    ...
    from factor_core.operator_complexity import get_op_complexity
    from factor_core.dimension import check_dimension_sanity
except ImportError:
    from src.alphagen.data.expression import Expression, Feature, Constant, DeltaTime, Operator
    from src.factor_core.operator_complexity import get_op_complexity
    from src.factor_core.dimension import check_dimension_sanity
...
class ExpressionQualityValidator:
    ...
    def _check_rules(self, report: ExpressionQualityReport):
        ...
        # 5. 量纲校验 (Dimension Sanity)
        if not check_dimension_sanity(expr):
            report.accept = False
            report.reason = "invalid_dimension"
            return
